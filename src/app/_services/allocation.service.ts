...
export class AllocationService {
  ...
  getAllocation(user_id: string) {
    ...
    return this.http
      .get(this.APIConfig.getUrl(this.API.GET_ALLOCATIONS, "UserId="+ user_id), httpOptions)
      .pipe((data: any) => {
        if (data) {
          return data;
        }
  ...
  getAllocationDetails(SONo: string) {
    // Add safe, URL encoded search parameter if there is a search term
    const httpOptions = {
      headers: new HttpHeaders({
        'Authorization': this.accessToken
    ...
    return this.http
      .get(this.APIConfig.getUrl(this.API.GET_ALLOCATION_SONO, "SONo="+ SONo), httpOptions)
      .pipe((data: any) => {
        if (data) {
          return data;
        }
