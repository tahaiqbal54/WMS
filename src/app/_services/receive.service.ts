...
export class ReceiveService {
  ...
  getReceive(user_id: string) {
    ...
    return this.http
      .get(this.APIConfig.getUrl(this.API.RECIEVING_LIST, "Userid="+ user_id), httpOptions)
      .pipe((data: any) => {
        if (data) {
          return data;
        }
  ...
  getReceiveDetails(PurchaseId: string) {
    // Add safe, URL encoded search parameter if there is a search term
    const httpOptions = {
      headers: new HttpHeaders({
        'Authorization': this.accessToken
    ...
    return this.http
      .get(this.APIConfig.getUrl(this.API.PURCASE_DETAIL_BYPURCHASEID, "PurchaseId="+ PurchaseId), httpOptions)
      .pipe((data: any) => {
        if (data) {
          return data;
        }
