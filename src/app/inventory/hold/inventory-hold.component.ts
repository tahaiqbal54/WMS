...
@Component({
  templateUrl: 'inventory-hold.html',
  providers: [InventoryService]
})
export class InventoryHoldComponent implements AfterViewInit, OnDestroy, OnInit {
...
Fetch(){
  if(this.WarehouseId == null || this.WarehouseId == undefined){
    ...
    this.InventoryService
    .getHoldData(this.WarehouseId,this.CustomerId,this.ProductId,this.BatchNo,this.LPNNo,this.LotNo)
    .subscribe(
      (data: any) => {
        console.log(data);
        this.users = data;
...
Holddata(){
  ...
  this.InventoryService.createHold(this.WarehouseId,this.reasonId,this.remarks,this.holdId)
      .subscribe(
        (data: any) => {
          if (data) {
            console.log(data);
